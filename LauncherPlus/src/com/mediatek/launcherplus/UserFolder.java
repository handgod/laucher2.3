        ...
        if (success) {
            ...
            if (Launcher.isShortcutScreen()) {
            	if (target instanceof Workspace) {
            		...
            		FolderIcon folderIcon = (FolderIcon)workspace.getViewForTag(mInfo);
            		folderIcon.updateAppIconThumbails();
            	} else if (target instanceof FolderIcon) {
            		if (!(target instanceof CellLayout)) {
            			target = (View) target.getParent();
            		}
            		FolderIcon folderIcon = null;
            		CellLayout cellLayout = (CellLayout) target;
            		int count = cellLayout.getChildCount();
            		for (int i = 0; i < count; i++) {
            			View child = cellLayout.getChildAt(i);
            			if (child.getTag() == mInfo) {
            				folderIcon = (FolderIcon) child;
            			}
            		}
            		if (folderIcon != null) {
            			folderIcon.updateAppIconThumbails();
