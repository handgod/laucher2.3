...
import com.mediatek.launcherplus.LauncherSettings;
...
public class LauncherModel extends BroadcastReceiver {
    ...
    static final String TAG = "LauncherPlus.Model";
    ...
    static private Launcher mLauncher;
    
    public static final String DATE_TIME_FORMAT = "yyyy-MM-dd HH:mm:ss";
    
    static final String SYSTEM_SCENE_WIDGET = "Widget"; 
    
    static final String SYSTEM_SCENE_SHORTCUT = "Shortcut";
    
    static final String[] SYSTEM_SCENE_NAMES = {
    	SYSTEM_SCENE_SHORTCUT, SYSTEM_SCENE_WIDGET
    };
    
    final static String APP_MANAGER_INSTALL_ACTION = "com.mediatek.intent.moreapp.APKINSTALL";
    private int mMoreAppPageType;
    private String mMoreAppPackageName = null;
    
    private ArrayList<LauncherAppWidgetInfo> mAppWidgets = new ArrayList<LauncherAppWidgetInfo>();
    ...
    static void moveItemInDatabase(Context context, ItemInfo item, long container, int screen,
        ...
        if ( Launcher.isDefStyle() ) {
        	values.put("scene", Launcher.defStyle);
        } else {
        	values.put("scene", Launcher.enhanceStyle);
        }
        ...
        if (Launcher.isInEditMode()) {
        	sWorker.post(new Runnable() {
                public void run() {
                	addItemToTempTable(tmpContext, tmpID); 
                }
            });
        	//addItemToTempTable(context, item.id);        	
        }
    ...
    private static void addItemToTempTable(Context context, long orininalID) {
    	final ContentResolver cr = context.getContentResolver();
    	Cursor c = cr.query(LauncherSettings.Favorites.getContentUri(orininalID, false), null, null, null, null);
    	int tmp_container = c.getColumnIndexOrThrow(LauncherSettings.Favorites.CONTAINER);
    	int tmp_screen = c.getColumnIndexOrThrow(LauncherSettings.Favorites.SCREEN);
    	int tmp_cellX = c.getColumnIndexOrThrow(LauncherSettings.Favorites.CELLX);
    	int tmp_cellY = c.getColumnIndexOrThrow(LauncherSettings.Favorites.CELLY);
    	int tmp_intent = c.getColumnIndexOrThrow(LauncherSettings.Favorites.INTENT);
    	int tmp_id = c.getColumnIndexOrThrow(LauncherSettings.Favorites._ID);
    	
    	c.moveToFirst();
    	
    	final ContentValues tmpValues= new ContentValues();
    	tmpValues.put(LauncherSettings.Favorites.CONTAINER, c.getLong(tmp_container));
    	tmpValues.put(LauncherSettings.Favorites.SCREEN, c.getInt(tmp_screen));
    	tmpValues.put(LauncherSettings.Favorites.CELLX, c.getInt(tmp_cellX));
    	tmpValues.put(LauncherSettings.Favorites.CELLY, c.getInt(tmp_cellY));
    	tmpValues.put(LauncherSettings.Favorites.INTENT, c.getString(tmp_intent));
    	tmpValues.put(LauncherSettings.Favorites.ORIGINAL_ID, c.getInt(tmp_id));
    	
    	cr.insert(LauncherSettings.Favorites.TEMP_CONTENT_URI, tmpValues);
    }
    ...
    static void addItemToDatabase(Context context, ItemInfo item, long container,
        ...
        final Context tmpContext = context;
        final ItemInfo tmpItem = item;
		final boolean tmpNotify = notify;
        if (Launcher.isInEditMode()) {
        	sWorker.post(new Runnable() {
        		public void run() {
        			addItemToTempTable(tmpContext, tmpItem, tmpNotify);
        		}
        	});
        }
    }
    
    static void addItemToTempTable(Context context, ItemInfo item, final boolean notify) {
    	final ContentResolver cr = context.getContentResolver();
    	if(item.itemType == LauncherSettings.Favorites.ITEM_TYPE_LIVE_FOLDER || 
    			item.itemType == LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER) {
    		final ContentValues tmpValues = new ContentValues();
    		tmpValues.put("container", item.container);
    		tmpValues.put("cellX", item.cellX);
    		tmpValues.put("cellY", item.cellY);
    		tmpValues.put("screen",item.screen);
    		tmpValues.put("originalID", item.id);
    		tmpValues.put("itemType", item.itemType);
    		cr.insert(notify ? LauncherSettings.Favorites.TEMP_CONTENT_URI : 
    			LauncherSettings.Favorites.TEMP_CONTENT_URI_NO_NOTIFICATION, tmpValues);
    	}
    }
     ...
     * @param item
     */
    static void deleteItemFromDatabase(Context context, long id) {
        final ContentResolver cr = context.getContentResolver();
     ...
     * Remove the contents of the specified folder from the database
     */
    static void deleteUserFolderContentsFromDatabase(Context context, long id) {
        final ContentResolver cr = context.getContentResolver();

        cr.delete(LauncherSettings.Favorites.getContentUri(id, false), null, null);
        cr.delete(LauncherSettings.Favorites.CONTENT_URI, LauncherSettings.Favorites.CONTAINER + "=" + id, null);
    }
        ...
        synchronized (mAllAppsListLock) {
            ...
            if (Intent.ACTION_PACKAGE_CHANGED.equals(action)
                ...
                if (Intent.ACTION_EXTERNAL_APPLICATIONS_AVAILABLE.equals(action)) {
                     ...
                     startLoader(context, false);
                } else if (APP_MANAGER_INSTALL_ACTION.equals(action)){
                	final Bundle extras = intent.getExtras();
                	mMoreAppPageType = (Integer)extras.get("pageType");
                	mMoreAppPackageName = (String)extras.get("packageName");
                }
            }
        }
    }
    ...
    private void addShortcut( Intent intent ) {
    	...
    	int screen = mLauncher.getShortcutWorkspace().getCurrentScreen();
    	if(mMoreAppPackageName!= null && mMoreAppPackageName.equals(packageName)){
    		screen = mMoreAppPageType+1;
    		mMoreAppPackageName = null;
    	}
    	
    	final List<ResolveInfo> matches = mAllAppsList.findActivitiesForPackage(mLauncher, packageName);

        if (matches.size() > 0) {
        	for (ResolveInfo info : matches) {
                ApplicationInfo applicationInfo = new ApplicationInfo(info, mIconCache);
                ...
                if ( componentName.getPackageName().equalsIgnoreCase(packageName)) {
                	...
                	shortcut.setComponent(componentName);
                	shortcut.putExtra(Intent.EXTRA_SHORTCUT_INTENT, new Intent(Intent.ACTION_MAIN).setComponent(componentName).addCategory(Intent.CATEGORY_LAUNCHER));
                	shortcut.putExtra(Intent.EXTRA_SHORTCUT_ICON, applicationInfo.iconBitmap);
                	
                	if (mLauncher.isInEditMode()) {
                		mLauncher.saveEditOperations();
                	}
                	mLauncher.completeAddApplication(mLauncher, shortcut, screen);
                }
            	break;
            }
        }
    ...
    public class Loader {
        ...
        final ArrayList<ItemInfo> mFolderItems = new ArrayList<ItemInfo>();
        ...
        private class LoaderThread extends Thread {
            ...
            private void loadWorkspace() {
                ...
                synchronized (LauncherModel.mObject) {
                    ...
                    final Cursor c = contentResolver.query(LauncherSettings.Favorites.CONTENT_URI,
                            null, "scene = '" + Launcher.getCurrentStyle() + "'", null, null);

                    ItemInfo shortcut_occupied[][][] = null;
                    ItemInfo widget_occupied[][][] = null;
                    
                    if ( mLauncher.isDefStyle() ) {
                    	shortcut_occupied = widget_occupied = new ItemInfo[Launcher.SCREEN_COUNT][Launcher.NUMBER_CELLS_X][Launcher.NUMBER_CELLS_Y];
                    } else {
                    	shortcut_occupied = new ItemInfo[Launcher.SCREEN_COUNT][Launcher.NUMBER_CELLS_X][Launcher.NUMBER_CELLS_Y];
                    	widget_occupied = new ItemInfo[Launcher.SCREEN_COUNT][Launcher.NUMBER_CELLS_X][Launcher.NUMBER_CELLS_Y];
                    }
                                ...
                                switch (itemType) {
                                    ...
                                    case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
                                        ...
                                        if (itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION) {
                                            if (container == LauncherSettings.Favorites.CONTAINER_PHONE){
                                        ...
                                        if (info != null) {
                                            ...
                                            // occupied
                                            if (!checkItemPlacement(shortcut_occupied, info)) {
                                                break;
                                            }

                                            switch (container) {
                                                case LauncherSettings.Favorites.CONTAINER_DESKTOP:
                                                case LauncherSettings.Favorites.CONTAINER_PHONE:
                                                case LauncherSettings.Favorites.CONTAINER_MESSAGE:
                                                    mItems.add(info);
                                                    break;
                                                default:
                                            ...
                                            // probably
                                            // because the
                                            // activity manager couldn't resolve
                                            // it
                                            // (maybe the app
                                            // was uninstalled), or the db row
                                            // was
                                            // somehow screwed up.
                                            // Delete it.
                                            ...
                                            Log.e(TAG, "Error loading shortcut " + id
                                                    + ", removing it");
                                            contentResolver.delete(LauncherSettings.Favorites
                                    ...
                                    case LauncherSettings.Favorites.ITEM_TYPE_USER_FOLDER:
                                        ...
                                        if (!checkItemPlacement(shortcut_occupied, folderInfo)) {
                                            break;
                                        }
                                        ...
                                        if (providerInfo == null && !isSafeMode) {
                                            ...
                                            if (intentDescription != null) {
                                                try {
                                                    ...
                                                    // Ignore, a live folder
                                                    // might
                                                    // not have a base intent
                                                }
                                            }
                                            ...
                                            // check & update map of what's
                                            // occupied
                                            if (!checkItemPlacement(shortcut_occupied, liveFolderInfo)) {
                                                break;
                                            }
                                    ...
                                    case LauncherSettings.Favorites.ITEM_TYPE_APPWIDGET:
                                        // Read all Launcher-specific widget
                                        // details
                                        ...
                                        if (!isSafeMode
                                            ...
                                            // check & update map of what's
                                            // occupied
                                            if (!checkItemPlacement(widget_occupied, appWidgetInfo)) {
                                                break;
                                            }
                    ...
                    if (DEBUG_LOADERS) {
                        Log
                                .d(TAG, "loaded workspace in " + (SystemClock.uptimeMillis() - t)
                                        + "ms");
                        ...
                        for (int y = 0; y < Launcher.NUMBER_CELLS_Y; y++) {
                            ...
                            for (int s = 0; s < Launcher.SCREEN_COUNT; s++) {
                                ...
                                for (int x = 0; x < Launcher.NUMBER_CELLS_X; x++) {
                                    line += ((shortcut_occupied[s][x][y] != null) ? "#" : ".");
                                }
                            }
                            Log.d(TAG, "[ " + line + " ]");
                        }
                        for (int y = 0; y < Launcher.NUMBER_CELLS_Y; y++) {
                            ...
                            for (int s = 0; s < Launcher.SCREEN_COUNT; s++) {
                                ...
                                for (int x = 0; x < Launcher.NUMBER_CELLS_X; x++) {
                                    line += ((widget_occupied[s][x][y] != null) ? "#" : ".");
                                }
                            }
                            Log.d(TAG, "[ " + line + " ]");
                        }
            ...
            private void bindWorkspace() {
				...
				//load App not on screen
                if ( !Launcher.isDefStyle() ) {
					mHandler.post(new Runnable() {
						public void run() {
							Callbacks callbacks = tryGetCallbacks(oldCallbacks);
							if (callbacks != null) {
								callbacks.bindAppNotInScreen(mFolderItems);
							}
						}
					});
                }
            ...
            public void dumpState() {
                ...
                Log.d(TAG, "mLoader.mLoaderThread.mLauncher=" + mLauncher);
     ...
     */
    void unbind() {
        unbindAppWidgetHostViews(mAppWidgets);
    }
