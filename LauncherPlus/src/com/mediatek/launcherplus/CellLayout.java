...
public class CellLayout extends ViewGroup {
	private static final String TAG = "LauncherPlus.CellLayout";
    ...
    public static final int DRAW_SCROLLING = 1;
    ...
    private Rect mCellForGreenRect = null;

    private RectF mTmpRectF = new RectF();

    private Rect mLastCellForGreenRect = new Rect();

    public static float sStrokeWidth;

    public static Paint sGreenRectPaint = null;
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		if (DisplayMetrics.DENSITY_DEVICE <= 120) {
        	mCellHeight = (int) (mCellHeight * 0.86f);
		}
        ...
        mMaxDistance = Math.sqrt(Math.pow(mCellWidth / 2, 2) + Math.pow(mCellHeight / 2, 2));
        if (sGreenRectPaint == null) {
            sGreenRectPaint = new Paint();
            sGreenRectPaint.setStyle(Style.STROKE);
            sGreenRectPaint.setAntiAlias(true);
            sStrokeWidth = 4.0f * getResources().getDisplayMetrics().density;
            sGreenRectPaint.setStrokeWidth(sStrokeWidth);
            sGreenRectPaint.setColorFilter(new PorterDuffColorFilter(Color.GREEN, PorterDuff.Mode.SRC_ATOP));
        }
    }

    @Override
    public void dispatchDraw(Canvas canvas) {
		final Animation ani = getAnimation();
		if (ani != null) {
			if (ani.hasStarted() && !ani.hasEnded()) {
				canvas.drawARGB(64, 216, 216, 216);
			}
		}
    ...
    protected void onDraw(Canvas canvas) {
        if (mDrawMode != DRAW_SCROLLING) {
            if (mDragging && mCellForGreenRect != null) {
                final float padding = (sStrokeWidth + 1) / 2;
                mTmpRectF.set(mCellForGreenRect.left + padding, mCellForGreenRect.top + padding,
                        mCellForGreenRect.right - padding, mCellForGreenRect.bottom - padding);
                canvas.drawRoundRect(mTmpRectF, 8f, 8f, sGreenRectPaint);
                mLastCellForGreenRect.set(mCellForGreenRect);
            } else {
                mLastCellForGreenRect.setEmpty();
            }
        }
    }

    public void setDrawState(int drawMode) {
        mDrawMode = drawMode;
    }
    ...
    public void setCellForGreenRect(Rect greenRect) {
        mCellForGreenRect = greenRect;
        if (greenRect != null) {
            invalidate(greenRect);
        }
        if (!mLastCellForGreenRect.isEmpty()) {
            invalidate(mLastCellForGreenRect);
        }
    }
	...
	private ArrayList<View> layoutOrder = new ArrayList<View>();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (lp.regenerateId) {
            	int id = ((getId() & 0xFF) << 16) | (lp.cellX & 0xFF) << 8 | (lp.cellY & 0xFF);
            	
            	if( !Launcher.isDefStyle() ) {
            		if( Launcher.isShortcutScreen()) {
            			id = id  & 0x0FFFFF | 0x100000;
            		} else {
            			id = id  & 0x0FFFFF | 0x200000;
            		}
            	}
...
out:            for (int i = x; i < x + spanX - 1 && x < xCount; i++) {
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public void setup(int cellWidth, int cellHeight, int widthGap, int heightGap,
            ...
            if (DEBUG_FLAG) {
            	Log.e(TAG, "setup x==" + x + " leftMargin==" + leftMargin);
            	Log.e(TAG, "setup y==" + y + " topMargin==" + topMargin + " width=" + width + " height=" + height);
            }
        }
    }
