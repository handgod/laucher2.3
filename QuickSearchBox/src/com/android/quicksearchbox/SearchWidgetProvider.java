...
import com.android.quicksearchbox.google.SearchEngineSettings;
...
public class SearchWidgetProvider extends BroadcastReceiver {
    ...
    /**
     * broadcast intent action for search engine change, immediately update web search hint
     * in search widget
    */
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (AppWidgetManager.ACTION_APPWIDGET_ENABLED.equals(action)) {
        	...
        	SharedPreferences p =
        		context.getSharedPreferences(SearchSettings.PREFERENCES_NAME, Context.MODE_PRIVATE);
        	QsbApplication.get(context).updateSearchEngineExternal(p, intent.getExtras().getString(SearchEngineSettings.PREF_SEARCH_ENGINE));
        	updateSearchWidgets(context);
    ...
    private static SearchWidgetState getSearchWidgetState(Context context, 
        ...
        if (corpus == null){
        	state.setQueryTextViewHint(context.getString(R.string.corpus_label_global));
		} else if (corpus.isWebCorpus()) {
			state.setQueryTextViewHint(QsbApplication.get(context).getSearchEngineInfo().getLabel());
		} else {
